...
```{r}
ggplot(athlete_data, aes(y=Days.since.Season.PR, x=as.factor(All.American),
                         fill=as.factor(All.American))) +
  geom_boxplot() +
  stat_summary(
    fun = mean,
    geom = "crossbar",
  ...
  labs(title="Days Since Season Record and All-American Status",
       x="All-American Status",
       y="Days since Season Record",
       caption = "*Purple bar displays the mean") + 
  scale_fill_discrete(name="All-American", 
                      labels=c("0"="No", "1"="Yes"))
```
